...
c_wave = 1/2*(cosh(k*meas_length)-cos(k*meas_length));
s_wave = 1/2*(sinh(k*meas_length)-sin(k*meas_length));
...
%6.29
X_m = c_wave-s_wave .* (C_wave ./ S_wave); % x = meas_length
...
%r = [0.021417677595425  -0.108957785556024   0.660008855293208];
%r = [-0.013559659419460   0.326652043660590   2.782184556484474];
%r = [0.133976673503339   5.409285813304700  -0.531100128195979];
r = [0.041157349254970  -0.195651626366985   2.517533512771213];
%r = [0.2205 -0.0001 -1.7087];
%r = ones(1,3)*0.0042;
...
freq_Hz = 20:20000;
%freq_Hz_2 = 20:0.009527678714905:20000+104*0.009527678714905; 
freq_Hz_2 = 20:0.009535850358740:20000; 
modes=3;
...
epsilon_u_piezo_2 = zeros(length(modes),length(freq_Hz_2));

for n = 1:modes;
    epsilon_u_piezo_2(n,:) = (1/Y)*(((n_m(n)*X_m(n)*X_m_d(n))*(1-((freq_Hz_2*2*pi)/w_m(n)).^2+1i*((freq_Hz_2*2*pi)/w_m(n))*(1/Q_m(n))))./...
        (((1-((freq_Hz_2*2*pi)/w_m(n)).^2).^2)+(((freq_Hz_2*2*pi)/w_m(n))*(1/Q_m(n))).^2));
end

epsilon_u_piezo_2 = sum(epsilon_u_piezo_2,1);
...
B_0 = 12 + 20*log10(abs(epsilon_u_piezo));
...
semilogx(f,20*log10(2*abs(Y_fft(1:NFFT/2+1))))
hold on;
%semilogx(B_0)
semilogx(f(1903:end),11 + 20*log10(abs(epsilon_u_piezo_2)))
...
hold on;
%semilogx(B_0)
plot(f(1903:end),10.^((11 + 20*log10(abs(epsilon_u_piezo_2)))./20))
title('Spectrum of chirp 20-20000Hz')
legend('Measurement','Model')

%%

%x = fminsearch(@(r) freq_func(r,freq_Hz_2,modes,w_m,Y,n_m,X_m,X_m_d,Y_fft,NFFT),[0.0042,0.3928,0.2226])
